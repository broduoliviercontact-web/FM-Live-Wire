...
// Story 3.1 — BrowserCompatGate unit tests (AD-3: feature detection ONLY).
//
// Proves:
...
//   - Missing Web MIDI API (secure) → terminal "Chrome/Edge requis".
...
import {
  BrowserCompatGate,
  detectBrowserCompat,
} from "../features/performer/components/BrowserCompatGate";
...
describe("detectBrowserCompat — pure feature detection", () => {
  it("insecure context + no Web MIDI → { false, false }", () => {
    setCompat({ secure: false, midi: false });
    expect(detectBrowserCompat()).toEqual({ secureContext: false, hasWebMidi: false });
  });

  it("secure context but no Web MIDI → secure true, midi false", () => {
    setCompat({ secure: true, midi: false });
    expect(detectBrowserCompat()).toEqual({ secureContext: true, hasWebMidi: false });
  });

  it("secure context + Web MIDI present → both true", () => {
    setCompat({ secure: true, midi: true });
    expect(detectBrowserCompat()).toEqual({ secureContext: true, hasWebMidi: true });
  });
});

describe("BrowserCompatGate — terminal screens", () => {
  ...
  it("secure but no Web MIDI → terminal 'Chrome/Edge requis'", () => {
    ...
    expect(screen.getByText("Chrome/Edge requis")).toBeInTheDocument();
    expect(screen.queryByText("SHOULD-NOT-RENDER")).not.toBeInTheDocument();
  });
  ...
  it("secure + Web MIDI → renders children", () => {
    ...
    render(
      <BrowserCompatGate>
        <span>REAL-PERFORMER-FLOW</span>
      </BrowserCompatGate>,
    );
    expect(screen.getByText("REAL-PERFORMER-FLOW")).toBeInTheDocument();
