...
> **Honnêteté de scope (consigne Story 6.8) :** ce document décrit la configuration de déploiement et les commandes à exécuter. Le déploiement réel sur un hôte prod + la terminaison TLS réelle ont été exécutés par Zub sur Render et signés en §14. `DEPLOYED_AND_VERIFIED` est attesté **uniquement** pour le périmètre 6.8 (déploiement prod HTTPS mono-domaine + env prod + secure context + flux end-to-end + zéro secret post-build + graceful shutdown). Le graceful shutdown est vérifié (☑ §14, restart Render observé le `2026-07-08`). Seul `/health` prod signé reste ☐ en §14 — à compléter lors de la prochaine vérification (3 états : idle / performer actif / listener rejoint). **S-10 / rehearsal reste `PENDING_ZUB_REHEARSAL_SIGNOFF`** : ce statut ne vaut pas `S10_PASS` / `MVP_LIVE_VALIDATED` (réservés à la répétition générale complète + première session audience réelle + rapport signé Zub).
...
- **Graceful shutdown vérifié :** ☑ — restart Render observé le `2026-07-08` : logs `{"name":"server","msg":"shutdown signal received, draining"}` → `{"name":"server","msg":"shutdown complete, exiting","meta":{"closed":true,"timedOut":false,"exitCode":0}}`, puis boot `==> Running 'node apps/server/dist/index.js'` → `{"name":"server","msg":"listening","meta":{"port":10000,"publicOrigin":"https://fm-live-wire.onrender.com"}}`. Drain côté client confirmé (`{"name":"socket","msg":"owner disconnected",...}` pendant le drain — `io.disconnectSockets(true)` a notifié le performer connecté). Aucune stacktrace, aucun `shutdown timeout`. Séquence reproductible sur un second restart.
